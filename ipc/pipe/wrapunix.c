 ...
 *warning: passing args 2 of `bind' discards `const' from pointer target type
 *warning: passing args 2 of `connect' discards `const' from pointer target type
 */

#include	"unpipc.h"

#include	<stdarg.h>		/* for variable args lists */
...
Semctl(int id, int semnum, int cmd, ...)
{
	...
	if (cmd == GETALL || cmd == SETALL || cmd == SETVAL ||
		...
		args = va_arg(ap, union semun);
		if ( (rc = semctl(id, semnum, cmd, args)) == -1)
			err_sys("semctl error");
		va_end(ap);
	} else {
